...
public class Vista {
	...
	public void mascheraInserimento(Persona p) {
		...
		do {
			flag=false;
			if(leggiStringa("inseire sesso").equalsIgnoreCase("f")) {
				p.setSesso(true);
			}else if(leggiStringa("inseire sesso").equalsIgnoreCase("m")) {
				p.setSesso(false);
			}else {
				flag=true;	
	...
	public void mascheraInserimentoDipendenti(Dipendente p) {
		...
		p.setSeriale(leggIntero("inserire seriale"));
		boolean flag=false;
		String s=leggiStringa("inseire sesso");
		do {
			flag=false;
		...
		p.setAzienda(leggiStringa("azienda"));
		p.setId_ruolo(1);
		p.setOreLavorative(leggiDouble("ore"));
		p.setStipendio(leggiDouble("stipendio"));
		if(p instanceof Manager) {
			Manager m=(Manager) p;
			p.setId_ruolo(2);
			m.setBenefit(leggiDouble("benefit"));
			m.setLivelloFunzionale(leggIntero("livello funzionale"));
		}
		if(p instanceof Dirigente) {
			Dirigente d=(Dirigente) p;
			p.setId_ruolo(3);
			d.setAreaResponsabilita(leggIntero("areaResponsabilita"));
		}
	}
